...
def CAGR(DF):
    ...
    df = DF.copy()
    if df.empty or len(df["cum_return"]) == 0:
        return 0.0  # Return 0 if the cumulative return list is empty
    else:
...
# while len(tickers) != 0 and attempt <=10:
#     for i in range(len(tickers)):
#         # ohlc_intraday[tickers[i]] = ts.get_intraday(symbol=tickers[i],interval='5min', outputsize='full')[0]
#         # print("Downloading data for {}",tickers[i])
#         # ohlc_intraday[tickers[i]] = yf.download(tickers[i], period='1mo', interval='5m')
